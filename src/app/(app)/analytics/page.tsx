...
export default async function AnalyticsPage() {
  ...
  return (
    <PageShell>
      ...
      />
      <div style={{ padding: '22px 22px 80px', maxWidth: 1100, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 22 }}>
        <section>
          ...
          <EditorialCard style={{ padding: 0 }}>
            <div className="overflow-x-auto">
              <table style={{ width: '100%', fontSize: 12, borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ background: 'var(--paper-2)', borderBottom: '1px solid var(--paper-ink)' }}>
                    {['Source', 'Applied', 'Screening', 'Interview', 'Offer', 'Rate'].map((h, i) => (
                      <th
                        key={h}
                        style={{
                ...
                <tbody>
                  {sourceRows.map(([src, counts], idx) => {
                    ...
                    const rateColor = rate > 5 ? 'var(--status-ok)' : rate >= 2 ? 'var(--status-warn)' : 'var(--s-rejected)'
                    return (
                      <tr key={src} style={{ borderTop: idx === 0 ? 'none' : '1px solid var(--border-soft)' }}>
                        <td style={{ padding: '10px 14px', color: 'var(--ink)', fontWeight: 500, textTransform: 'capitalize' }}>{src}</td>
                        <td style={{ padding: '10px 14px', textAlign: 'right', color: 'var(--ink-3)', fontFamily: 'var(--font-mono)' }}>{counts.applied}</td>
                        <td style={{ padding: '10px 14px', textAlign: 'right', color: 'var(--ink-3)', fontFamily: 'var(--font-mono)' }}>{counts.screening}</td>
                        <td style={{ padding: '10px 14px', textAlign: 'right', color: 'var(--ink-3)', fontFamily: 'var(--font-mono)' }}>{counts.interview}</td>
                        <td style={{ padding: '10px 14px', textAlign: 'right', color: 'var(--ink-3)', fontFamily: 'var(--font-mono)' }}>{counts.offer}</td>
                        <td style={{ padding: '10px 14px', textAlign: 'right', color: rateColor, fontWeight: 500, fontFamily: 'var(--font-mono)' }}>{rate}%</td>
                      </tr>
                    )
                  })}
                </tbody>
          ...
          <EditorialCard>
            {avgDaysToInterview === null && avgDaysToOffer === null ? (
              <p style={{ fontSize: 13, color: 'var(--ink-4)' }}>
                No resolved data yet — stats appear once you log completed interview or offer events.
              </p>
            ) : (
              <div style={{ display: 'flex', gap: 32, flexWrap: 'wrap' }}>
                {avgDaysToInterview !== null && (
                  <div>
                    <p style={{ fontFamily: 'var(--font-serif)', fontSize: 36, fontWeight: 500, color: 'var(--accent-ij-ink)' }}>
                      {avgDaysToInterview}d
                    </p>
                    <p style={{ fontSize: 12, color: 'var(--ink-4)', marginTop: 4 }}>Avg. applied → first interview</p>
                  </div>
                )}
                {avgDaysToOffer !== null && (
                  <div>
                    <p style={{ fontFamily: 'var(--font-serif)', fontSize: 36, fontWeight: 500, color: 'var(--ink)' }}>
                      {avgDaysToOffer}d
                    </p>
                    <p style={{ fontSize: 12, color: 'var(--ink-4)', marginTop: 4 }}>Avg. applied → offer</p>
                  </div>
                )}
              </div>
            )}
