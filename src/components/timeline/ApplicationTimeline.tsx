...
const EVENT_COLORS: Record<string, string> = {
  ...
  screening_scheduled: 'bg-yellow-500',
  screening_completed: 'bg-yellow-400',
  interview_scheduled: 'bg-blue-500',
  interview_completed: 'bg-blue-400',
  technical_assessment: 'bg-blue-600',
  offer_received: 'bg-purple-500',
  offer_accepted: 'bg-green-500',
  offer_declined: 'bg-red-400',
  offer_rescinded: 'bg-red-600',
  rejected: 'bg-red-500',
  ...
  if (events.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center">
        ...
        <p className="mt-1 text-sm text-slate-500">Add an event or drop a document to get started.</p>
      </div>
    )
  }
  ...
  return (
    <div className="relative space-y-4 pl-8">
      ...
      {events.map((event, i) => {
        ...
        const color = EVENT_COLORS[event.event_type] ?? 'bg-slate-500'
        ...
        return (
          <div key={event.id} className="relative flex gap-4">
            {/* Dot */}
            <div
              className={`absolute -left-5 flex h-6 w-6 items-center justify-center rounded-full ${color} shrink-0 ring-2 ring-slate-900`}
            >
              <Icon className="h-3 w-3 text-white" />
            </div>

            {/* Content */}
            <div className="flex-1 rounded-lg border border-slate-700 bg-slate-800 p-4">
              <div className="flex items-start justify-between gap-2">
                ...
                <div className="flex flex-col items-end gap-1 shrink-0">
                  <Badge
                    variant="outline"
                    className="border-slate-600 text-xs text-slate-400 capitalize"
                  >
                    {event.event_type.replace(/_/g, ' ')}
                  </Badge>
                  {event.source === 'ai_parsed' && (
                    <Badge className="bg-blue-600/20 text-xs text-blue-400">AI</Badge>
                  )}
                </div>
              </div>
