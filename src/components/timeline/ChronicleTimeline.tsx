...
const EVENT_COLORS: Record<string, { dot: string; border: string }> = {
  ...
  reference_check:       { dot: 'bg-indigo-500',  border: 'border-l-indigo-500' },
  nda_signed:            { dot: 'bg-indigo-400',  border: 'border-l-indigo-400' },
        ...
        {events.map((event) => {
          ...
          return (
            <div key={event.id} className="flex items-start gap-4">
              {/* Dot */}
              <div
                className={`w-10 h-10 rounded-full flex-shrink-0 flex items-center justify-center ${colors.dot} shadow-sm`}
              >
              ...
              {/* Card */}
              <div
                className={`flex-1 bg-white rounded-xl p-5 border border-slate-100 border-l-4 ${colors.border} shadow-sm hover:shadow-md transition-shadow`}
              >
                <div className="flex items-start justify-between gap-2">
                  ...
                  <div className="flex flex-col items-end gap-1 shrink-0">
                    <Badge
                      variant="outline"
                      className="text-xs text-slate-500 border-slate-200 capitalize whitespace-nowrap"
                    >
                      {event.event_type.replace(/_/g, ' ')}
                    </Badge>
                    {event.source === 'ai_parsed' && (
                      <Badge className="bg-blue-50 text-xs text-blue-600 border-blue-100">
                        AI
                      </Badge>
                    )}
                  </div>
